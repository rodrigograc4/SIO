...
const Produtos_Cards = ({ item }) => {
    ...
    const sanitizedTipo = DOMPurify.sanitize(editedItem.tipo);
    const sanitizedNome = DOMPurify.sanitize(editedItem.nome);
    const sanitizedDescricao = DOMPurify.sanitize(editedItem.descricao);
    const sanitizedTamanho = DOMPurify.sanitize(editedItem.tamanho);
    
    //console.log("Sanitized:", sanitizedTipo, sanitizedNome, sanitizedDescricao, sanitizedTamanho)
    ...
    const handleSaveClick = async () => {
        ...
        //const validatedTipo = validateInput(sanitizedTipo);
    ...
    const handleCartClick = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          console.error('Invalid quantity. The item was not added to the cart.');
          alert('Quantidade excede o stock. Quantidade em stock: '+ item.Nstock + ' unidades');
        } else {
