...
pool.connect()
  ...
  app.post('/api/register', async (req, res) => {
    ...
    try {
      ...
      const result = await request.query(
        'INSERT INTO Deti.users (username, email, senha, salt, foto_url, isAdmin) VALUES (@username, @email, @password, @salt, @foto_url, 0)',
      );
...
app.post('/api/reset-password', async (req, res) => {
  ...
  try {
    ...
    if (result.recordset.length === 1) {
      ...
      const hashedNewPassword = hashNewPassword(newPassword, salt); 
      ...
      const updateResult = await updateRequest.query(
        'UPDATE Deti.users SET senha = @newPassword, salt = @salt, passdate = GETDATE() WHERE username = @username'
      );
